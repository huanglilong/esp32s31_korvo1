...
#define CONFIG_APP_WIFI_AP_SSID_PREFIX "esp-s31"
#endif
...
static void compose_ap_ssid(void)
{
    ...
    uint8_t mac[6] = {0};
#ifdef CONFIG_IDF_TARGET_ESP32P4
    /* On P4, the C6 coprocessor provides WiFi via esp_hosted.
     * esp_hosted_iface_mac_addr_get(WIFI_SOFTAP) may fail with RPC timeout
     * because C6 doesn't expose a separate SoftAP MAC. Fall back to
     * P4's own eFuse MAC (used by the mDNS hostname as well). */
    ...
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "esp_hosted_iface_mac_addr_get(SOFTAP) failed, using eFuse MAC: %s",
                 esp_err_to_name(ret));
