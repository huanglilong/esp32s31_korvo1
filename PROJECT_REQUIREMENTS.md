...
| 2026-07-17 | v0.11.2 | **Fix MP3 playback slow at 16kHz**: Root cause — `CONFIG_AUDIO_SIMPLE_PLAYER_RESAMPLE_DEST_RATE` was 48000 in sdkconfig (stale from 48kHz test) while I2S/codec ran at 16000Hz. Rate converter amplified 44.1kHz→48kHz (3× the I2S rate), causing GMF pipeline backup → extreme slow playback. **Fix**: set `RESAMPLE_DEST_RATE=16000` in both `sdkconfig.defaults` and `sdkconfig`. **⚠️ IMPORTANT: after changing sdkconfig.defaults, MUST run `idf.py fullclean && idf.py build` to regenerate sdkconfig**. Also removed dead code (`i2s_write_direct`, `tx_handle`/`rx_handle`) from AudioDriver. Restored `_asp_out` to use `codec_write()` (was broken `i2s_write_direct`). `no_dac_ref=true` tested and confirmed NOT needed — reverted. Display/Camera enabled (not related). All sample rates unified to 16kHz per BSP recommendation.
