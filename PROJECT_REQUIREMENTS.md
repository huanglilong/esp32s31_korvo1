...
| C2 | **HMI 图形界面** | ESP-Brookesia Phone UI 桌面 + 自定义 App (音乐播放器/设置/录音) | 🟡 基线已接入，System Super 与自定义 App 待迁移 |
...
| 2026-07-19 | v0.12.0 | **ESP-Brookesia master/v0.8 基线集成**: ESP-IDF 6.2 兼容性确认；加入 Brookesia Display/Audio/Video Service 与 LVGL 组件依赖及编译边界，补充 LVGL 文件系统盘符配置。由于 Brookesia HAL Boards 当前要求 `esp_board_manager 0.5.*`，而项目生成代码使用 0.6.x，HAL/系统所有权迁移暂缓；现有 WiFi、音频、Camera 驱动保持运行。临时本地 checkout 构建通过；官方 Git 依赖构建受 GitHub 网络超时影响，待网络可用复验。 |
| 2026-07-20 | v0.12.1 | **Code Review Round 1 — 5 项修复**: (1) 修复 `_api_rec_start` 编码器缓冲区分配失败时 `s_enc_in_buf` 内存泄漏；(2) 修复 `_api_files_list` 3 个错误路径缺失 `Content-Type: application/json`；(3) 修复 `_api_rec_status` 非录音时返回 `{"recording":0}` 整数而非布尔值的不一致问题；(4) 修复 `_api_files_list` 重复 `opendir("/sdcard")` 调用；(5) **修复 `APP_BROOKESIA_ENABLE=y` 时启动崩溃** — Brookesia 和 legacy WiFi 都调用 `esp_netif_init()` 第二次触发 `ESP_ERROR_CHECK` abort。修复：`brookesia_app.cpp` 和 `wifi_manager.c` 中 `ESP_ERROR_CHECK(esp_netif_init())` 改为优雅处理 `ESP_ERR_INVALID_STATE`。另将 WiFi 初始化移出 `!s_brookesia_owns_hardware` 块，确保 WiFi 始终运行（Web 配网和测试需要）。Build 通过，Flash 成功，设备启动正常，AP SSID: `esp-s31-f400bc`。 |
| 2026-07-20 | v0.12.2 | **Fix SD card mount failure when Brookesia owns hardware**: Root cause — when `APP_BROOKESIA_ENABLE=y`, `brookesia_app_start()` returns true → `s_brookesia_owns_hardware=true` → `SDCardDriver::init()` never called → `SDCardDriver::available()` returns false → all SD card web API endpoints report "SD card not available" → pytest test_files/test_system SD card tests fail. **Fix**: (1) Add `SDCardDriver::detect_mount()` method that checks if `/sdcard` VFS mount point is already accessible (mounted by Brookesia's StorageFileSystemImpl via esp_board_manager), and marks the driver as initialized without calling `bsp_sdcard_mount()` again. (2) In `main.cpp`, call `detect_mount()` in the `s_brookesia_owns_hardware` else branch so `SDCardDriver::available()` returns true. Build 通过。 |
| 2026-07-21 | v0.12.3 | **Fix audio and WiFi when Brookesia owns hardware**: (1) **Audio not available**: Same pattern as SD card — `AudioDriver::init()` never called when Brookesia owns hardware. Fix: Add `AudioDriver::detect_brookesia_audio()` that initializes DAC/ADC via `esp_board_manager_init_device_by_name()` and retrieves codec handles, so web API audio endpoints work. (2) **WiFi connect retries too low**: Brookesia WiFi service had `CONNECT_RETRIES_MAX=3` which was insufficient for WPA3-SAE auth that can fail intermittently. Fix: Increase to `10` retries and `5s` auto-connect delay. Build 通过, Flash 通过, SD card + audio + volume APIs verified working. |
