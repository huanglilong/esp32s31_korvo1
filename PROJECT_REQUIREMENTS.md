...
| 2026-07-16 | v0.10.3 | **Replace shine MP3 with native PCM WAV recording**: (1) Remove shine_encoder component entirely. (2) New WAV/PCM recording: I2S RX → raw PCM → SD card with proper WAV header (44 bytes, auto-patched on stop). (3) Simpler audio_task: direct fwrite of PCM data, no encoder buffering. (4) `esp_audio_simple_player` natively supports WAV playback. (5) API list/play updated for .wav files. Recording test: 5s→353KB WAV (44B header + 353KB PCM). Build + Flash 通过。 |
