...
| 2026-07-15 | v0.10.2 | **Fix: Audio recording MP3 无声音 — 4个根因修复**: **(1) 采样率**: 22050→16000 Hz。ESP32-S31 MCLK 未连接, BSP 推荐 16kHz。22050 Hz 时 ES8389 ADC 输出全零, 16000 Hz 正常。(2) **PGA Mic Gain**: 默认 0 dB→40 dB (ES8389 最大硬件增益 36.5 dB)。参考 BSP `display_audio_photo` 示例 `esp_codec_dev_set_in_gain(50.0)`。(3) **`esp_codec_dev_read/write` API 语义**: 返回状态码(0=OK)非字节数, `AudioDriver::codec_read/write` 修正为返回 size。(4) **`mic_dev` 分离 handle**: `codec_read` 使用 `mic_dev`, `set_mic_gain` 使用 `mic_dev`。录音测试: 8s→132KB MP3, 40dB mic gain。Build + Flash 通过。 |
