...
| 2026-07-20 | v0.12.1 | **Code Review Round 1 — 4 项修复**: (1) 修复 `_api_rec_start` 编码器缓冲区分配失败时 `s_enc_in_buf` 内存泄漏；(2) 修复 `_api_files_list` 3 个错误路径缺失 `Content-Type: application/json`；(3) 修复 `_api_rec_status` 非录音时返回 `{"recording":0}` 整数而非布尔值的不一致问题；(4) 修复 `_api_files_list` 重复 `opendir("/sdcard")` 调用。Build 通过。 |
