...
- **Camera**: DVP OV3660 摄像头驱动 (optional, mutex 互斥控制), Camera App ULog 持续录制 (V4L2 → PPA downscale+rotate180° → HW JPEG → camera_frame uORB)
...
- **ULog**: PX4 ULog 格式二进制日志 (SD 卡 .ulg 文件, 含 git branch/commit/author/date/msg, SNTP 同步后自动启动, 持续 Audio 录制 AAC-ADTS → audio_frame uORB topic, Camera JPEG 帧录制 → camera_frame uORB topic, PPA 180°旋转+缩放+byte-swap)
